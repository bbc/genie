  ...
  for (var j = 0, jcnt = rows.length; j < jcnt; j++) {
    ...
    for (var i = 0, icnt = columns.length; i < icnt; i++) {
      ...
      if (colWidth >= 1 && rowHeight >= 1) {
        ...
        console.warn("Size of Grid(".concat(i, ",").concat(j, ") = ").concat(colWidth, "x").concat(rowHeight, ", which is invalid"));
      }
...
var NinePatch = /*#__PURE__*/function (_RenderTexture) {
  ...
  function NinePatch(scene, x, y, width, height, key, baseFrame, columns, rows, config) {
    ...
    _this.setGetFrameNameCallback(GetValue$1(config, 'getFrameNameCallback', undefined));

    _this.setStretchMode(GetValue$1(config, 'stretchMode', 0));
...
Object.assign(NinePatch.prototype, TextureMethods);
