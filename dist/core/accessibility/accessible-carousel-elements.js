 ...
 * @license Apache-2.0
 */
import { accessibleDomElement } from "./accessible-dom-element.js"; //Should be in a stylesheet however we have no support for this (v0.4)
...
export function create(pageName, carouselSprites, parentElement, choices) {
  ...
  var carousel = createCarouselElement(parentElement, pageName); // NOTE: Hack to force tab accessibility order
  // needs a refactor once we have an accessibility layer

  carouselSprites.forEach(function (sprite, index) {
    var accessibleElement = accessibleDomElement({
